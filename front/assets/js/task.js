const taskManager = {
    ...
    fetchAndInsertTasksFromApi: async function (event) {
        ...
        tasks.forEach(taskManager.insertTaskInHtml);
        
        // Boucle sur la liste des tâches
        async function fetchTasks() {
            const httpResponse = await fetch(taskManager.apiTaskEndpoint);
    ...
    handleCreateForm: async function (event) {
        ...
        // Étape 4: Après confirmation de l'API insérer la tâche dans la page 
    ...
    handleDeleteButton: async function (event) {
        ...
        const url = `${taskManager.apiEndpoint}/${taskId}`;
    ...
    handleEditForm: function (event) {
        ...
        const url = `${taskManager.apiTaskEndpoint}/${taskId}`;
